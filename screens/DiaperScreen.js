...
export default function DiaperScreen({ baby, onAddDiaper, onUpdateDiaper, onDeleteDiaper }) {
  ...
  return (
    <ScrollView style={styles.page} contentContainerStyle={styles.contentContainer}>
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>排便记录</Text>
        <View style={styles.row}>
          {['大便', '小便'].map((item) => (
            <Pressable key={item} onPress={() => setDiaperType(item)} style={[styles.chip, diaperType === item && { backgroundColor: themeColor }]}> 
              <Text style={[styles.chipText, diaperType === item && styles.chipActiveText]}>{item}</Text>
            </Pressable>
          ))}
        </View>
        ...
        <View style={styles.fieldRow}>
          ...
          <TextInput style={styles.fieldInput} value={color} onChangeText={setColor} placeholder="如 金黄" />
        </View>
        <View style={styles.fieldRow}>
          <Text style={styles.fieldLabel}>备注</Text>
          <View style={styles.noteRow}>
            <TextInput style={[styles.fieldInput, styles.textArea, styles.noteInput]} value={note} onChangeText={setNote} placeholder="如 有少量奶瓣" multiline />
        ...
        <Pressable style={[styles.saveButton, { backgroundColor: themeColor }]} onPress={submit}>
          <Text style={styles.saveText}>{isEditing ? '保存修改' : '保存排便记录'}</Text>
        </Pressable>
        {isEditing && (
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>排便历史</Text>
        {baby.diaperRecords.length === 0 ? (
          <Text style={styles.emptyText}>暂无排便记录。</Text>
        ) : (
          baby.diaperRecords.map((item) => (
            <View key={item.id} style={styles.recordCard}>
              <View style={styles.recordRow}>
                <View style={styles.recordContent}>
                  <Text style={styles.recordTitle}>{item.startTime}</Text>
                  <Text style={styles.recordText}>{item.type} · {item.color}</Text>
